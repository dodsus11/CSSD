...
Public Class ucDistribusiRuanganDaftar
    ...
    Sub loadData()
        ...
        dt = Me.cssdServ.GetAllDistribusiRuangan(dtFrom.Value, dtTo.Value)
        ...
        col.Add(GH.AddModelGridColumn("Petugas Pengirim", 120))
        col.Add(GH.AddModelGridColumn("Petugas Penerima", 130))
        col.Add(GH.AddModelGridColumn("Status", 70))
        GH.FormatGrid(dgvDaftar, dt, col, False, True, False, False)
        'GH.FormatGrid(dgvDaftar, dt, col, False, True, True, False)
        'GH.gridControl(dgvDaftar, GH.GridControlType.Button, "Cetak Form", Color.DarkOliveGreen, dgvDaftar.ColumnCount)
        'GH.gridControl(dgvDaftar, GH.GridControlType.Button, "Batal Distribusi", Color.Crimson, dgvDaftar.ColumnCount)
    ...
    Private Sub dgvDaftar_CellContentClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvDaftar.CellContentClick
        'Dim cetakIndex As Integer = dgvDaftar.Columns("Cetak Form").Index
        'Dim batalIndex As Integer = dgvDaftar.Columns("Batal Distribusi").Index
        ...
        '    dt = Me.cssdServ.GetReportBuktiDistribusi(dgvDaftar.DataSource.rows(e.RowIndex).item(0))
        ...
        'If e.ColumnIndex = batalIndex Then
        ...
        '    datamaster.IdOrder = dgvDaftar.DataSource.rows(e.RowIndex).item(0)
        '    datamaster.IdOrderDetail = dgvDaftar.DataSource.rows(e.RowIndex).item(1)
        '    datamaster.NoTracing = dgvDaftar.DataSource.rows(e.RowIndex).item(3)
        '    datamaster.NoInventory = dgvDaftar.DataSource.rows(e.RowIndex).item(4)
